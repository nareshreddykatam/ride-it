...
if (firebaseConfig.apiKey) {
  ...
  if (messaging) {
    messaging.onBackgroundMessage((payload) => {
      const title = (payload.notification && payload.notification.title) || "Ride It";
      const body = (payload.notification && payload.notification.body) || "";
      const deepLink = (payload.data && payload.data.deep_link) || "/home";
...
self.addEventListener("notificationclick", (event) => {
  ...
  const deepLink = (event.notification.data && event.notification.data.deepLink) || "/home";
  event.waitUntil(
