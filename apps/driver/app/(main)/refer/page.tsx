...
export default function DriverReferPage() {
  ...
  const pendingCount = referrals.filter((r) => r.status === "attributed" || r.status === "qualified").length;
        ...
        <>
          <Card tone="tinted" className="mt-6 text-center">
            ...
            <p className="mt-3 text-xs text-ink-soft">
              ...
              {referrals.some((r) => r.invitee_role === "driver") ? " (drivers also need to complete verification)" : ""}.
            </p>
          </Card>

          <div className="mt-4 grid grid-cols-3 gap-2.5">
            <Card className="text-center">
              <p className="font-display text-xl font-bold text-ink">{summary?.totalReferrals ?? 0}</p>
              <p className="mt-0.5 text-[11px] text-ink-soft">Total</p>
            </Card>
            <Card className="text-center">
              <p className="font-display text-xl font-bold text-marigold-text">{pendingCount}</p>
              <p className="mt-0.5 text-[11px] text-ink-soft">Pending</p>
            </Card>
            <Card className="text-center">
              <p className="font-display text-xl font-bold text-meter-green-text">₹{summary?.totalRewardsEarned ?? 0}</p>
              <p className="mt-0.5 text-[11px] text-ink-soft">Earned</p>
            </Card>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between rounded-lg border border-border bg-surface px-4 py-3 text-sm">
            <span className="flex items-center gap-2 text-ink-soft">
              <Users size={14} /> Passenger referrals
            </span>
          ...
          <div className="mt-2 flex items-center justify-between rounded-lg border border-border bg-surface px-4 py-3 text-sm">
            <span className="flex items-center gap-2 text-ink-soft">
              <Users size={14} /> Driver referrals
            </span>
            <span className="font-medium text-ink">{summary?.driverReferrals ?? 0}</span>
          </div>

          <p className="mb-2 mt-6 text-xs font-semibold uppercase tracking-wide text-ink-soft">Referral activity</p>
          {referrals.length === 0 ? (
