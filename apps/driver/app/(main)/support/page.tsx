...
export default function SupportPage() {
  ...
  const [category, setCategory] = React.useState<string>(DRIVER_SUPPORT_CATEGORIES[0].value);
  ...
  function openNewTicket() {
    setCategory(DRIVER_SUPPORT_CATEGORIES[0].value);
    setDescription("");
    setSubmitted(false);
    setSheetOpen(true);
  }
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const chosen = DRIVER_SUPPORT_CATEGORIES.find((c) => c.value === category) ?? DRIVER_SUPPORT_CATEGORIES[0];
  ...
  return (
    <main className="flex-1 px-6 py-8">
      ...
      <p className="mt-1 text-sm text-ink-soft">Earnings, verification, account, and app problems.</p>
          ...
          <>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <Select label="What's this about?" size="sm" value={category} onChange={(e) => setCategory(e.target.value)}>
                {DRIVER_SUPPORT_CATEGORIES.map((c) => (
