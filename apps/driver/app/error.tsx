 ...
 * driver a way back instead of a dead screen. `reset()` re-renders the
 * segment (useful for transient failures); "Go to Dashboard" is the
 * fallback for anything reset() can't clear on its own.
 */
export default function ErrorBoundary({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  React.useEffect(() => {
    console.error("[driver] Unhandled render error:", error);
  }, [error]);
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-4 px-6 py-10 text-center">
      ...
      <p className="text-sm text-ink-soft">
        This screen ran into a problem. You can try again, or head back to Dashboard.
      </p>
