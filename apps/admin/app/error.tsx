 ...
 * admin a way back instead of a dead screen. `reset()` re-renders the
 * segment (useful for transient failures); "Go to Overview" is the
 * fallback for anything reset() can't clear on its own.
 */
  ...
  return (
    <main className="flex min-h-dvh flex-col items-center justify-center gap-4 px-6 py-10 text-center">
      ...
      <p className="text-sm text-ink-soft">
        This screen ran into a problem. You can try again, or head back to Overview.
      </p>
      <div className="mt-2 flex w-full max-w-xs flex-col gap-2">
        <Button className="w-full" onClick={reset}>
          Try again
        </Button>
        <Button variant="outline" className="w-full" onClick={() => (window.location.href = "/overview")}>
          Go to Overview
        </Button>
      </div>
    </main>
